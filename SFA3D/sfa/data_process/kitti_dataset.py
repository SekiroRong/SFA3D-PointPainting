...
class KittiDataset(Dataset):
    def __init__(self, configs, mode='train', lidar_aug=None, hflip_prob=None, num_samples=None):
        ...
        if PP:
            self.lidar_dir = os.path.join(self.dataset_dir, sub_folder, "paintedVelodyne")
            if pure:
        ...
        if PP:
            self.sample_id_list = [x for x in open(split_txt_path).readlines()]
        else:
            self.sample_id_list = [int(x.strip()) for x in open(split_txt_path).readlines()]
    ...
    def load_img_only(self, index):
        ...
        sample_id = self.sample_id_list[index].strip()
        # print(sample_id)
        img_path, img_rgb = self.get_image(sample_id)
    ...
    def load_img_with_targets(self, index):
        ...
        bev_map = makeBEVMap(lidarData, cnf.boundary, mode='train')
        # print(bev_map)
        # pause()
        bev_map = torch.from_numpy(bev_map)
    ...
    def get_image(self, idx):
        ...
        # img_path2 = os.path.join(self.image_dir, '{:06d}_10.png'.format(idx))
        if PP:
            img_path = os.path.join(self.image_dir, '{}.jpg'.format(idx))
        else:
    ...
    def get_calib(self, idx):
        if PP:
            calib_file = r'G:\PP\carla\training\modified_calib_file.txt'
        else:
    ...
    def get_lidar(self, idx):
        ...
        if PP:
            ...
            # print(lidar_file)
            # print(np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4))
            # pause()
...
if __name__ == '__main__':
    ...
    from data_process.transformation import OneOf, Random_Scaling, Random_Rotation, lidar_to_camera_box, Semantic_Jit
    ...
    print('configs.dataset_dir = ',configs.dataset_dir)
    # lidar_aug = OneOf([
    ...
    for idx in range(len(dataset)):
        ...
        img_rgb = show_rgb_image_with_boxes(img_rgb, labels, calib,inference=False)
