...
def lidar_to_camera_box(boxes, V2C=None, R0=None, P2=None):
    ...
    labels = np.array(ret).reshape(-1, 7)
    if PP:
        labels = labels[:, [1, 2, 0, 3, 4, 5, 6]]
        # labels[:, 0] *= -1
        # labels[:, 2] *= -1
        labels[:, 6] *= -1
    return labels
...
class OneOf(object):
    ...
    def __call__(self, lidar, labels):
        if np.random.random() <= self.p:
            ...
            # lidar, labels = Semantic_Jit()(lidar, labels)
...
import time
class Semantic_Jit(object):
    def __init__(self, p=0.5, class_num=2):
        self.p = p
        self.p_sub = 1/class_num
    ...
    def __call__(self, lidar, labels):
        ...
        # print(lidar.shape)
        noise_num = int(self.p_sub*self.p*length)
        # print(length, noise_num)
        index_v = np.random.choice(length, noise_num,replace=False)
        index_p = np.random.choice(length, noise_num, replace=False)
        lidar[index_p, 4] = 2
        lidar[index_v, 4] = 8
        # t2 = time.time()
        # print(t2-t1)
