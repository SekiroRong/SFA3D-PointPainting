...
from utils.config import PP, step, nothing
...
def parse_test_configs():
    ...
    parser.add_argument('--pretrained_path', type=str,
                        default=r'G:\PP\carla\checkpoints\pure/Model_pure_epoch_8_0.483_0.758.pth', metavar='PATH',
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_dataloader):
            ...
            if nothing:
                bevmaps_lite = bev_maps[:, :2, :, :]
                input_bev_maps = bevmaps_lite.to(configs.device, non_blocking=True).float()
            else:
            ...
            result_filename = os.path.join(r'G:\PP\carla\testing', 'result', metadatas['img_path'][0][28:-4]+'.txt')
            # print(bev_map)
            ...
            out_img = merge_rgb_to_bev(img_bgr, bev_map[:,:,:3], output_width=configs.output_width)

            print('\tDone testing the {}th sample, time: {:.1f}ms, speed {:.2f}FPS'.format(batch_idx, (t2 - t1) * 1000,
                                                                                           1 / (t2 - t1)))
            cv2.putText(out_img, "FPS:{:.2f}".format(1 / (t2 - t1)), (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
                        (0, 0, 255), 1)
            if True:
