...
from IoU import iou_rotate_calculate
...
GT_PATH = os.path.join(root_dir,'label_2')
DR_PATH = os.path.join(root_dir,'result')
# if there are no images then no animation can be shown
IMG_PATH = os.path.join(root_dir,'image_2')
    ...
    for line in lines_list:
        try:
            if "difficult" in line:
                    class_name, x, y, z, h, w, l, ry, a, b, c, d, _difficult = line.split()
                    is_difficult = True
            else:
                    class_name, x, y, z, h, w, l, ry, a, b, c, d = line.split()
...
for class_index, class_name in enumerate(gt_classes):
    ...
    for txt_file in dr_files_list:
        ...
        for line in lines:
            try:
                tmp_class_name, confidence, x, y, w, l, yaw = line.split()
            except ValueError:
                error_msg = "Error: File " + txt_file + " in the wrong format.\n"
...
with open(output_files_path + "/output.txt", 'w') as output_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            bb = np.array([ float(x) for x in detection["bbox"].split() ])
            for obj in ground_truth_data:
                # look for a class_name match
                if obj["class_name"] == class_name:
                    bbgt = np.array([ float(x) for x in obj["bbox"].split() ])
                    # print(bbgt)
                    ov = iou_rotate_calculate(bb,bbgt)
                    # print(bbgt)
                    # print(bb)
                    # print(ov)
                    # print(' ')
