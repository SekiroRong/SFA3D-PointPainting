...
def save_checkpoint(checkpoints_dir, saved_fn, model_state_dict, utils_state_dict, epoch, val_loss, train_loss):
    """Save checkpoint every epoch only is best model or after every checkpoint_freq epoch"""
    model_save_path = os.path.join(checkpoints_dir, 'Model_{}_epoch_{}_{:.3f}_{:.3f}.pth'.format(saved_fn, epoch, train_loss, val_loss))
    utils_save_path = os.path.join(checkpoints_dir, 'Utils_{}_epoch_{}_{:.3f}_{:.3f}.pth'.format(saved_fn, epoch, train_loss, val_loss))
