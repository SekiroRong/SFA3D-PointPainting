...
def decode(hm_cen, cen_offset, direction, z_coor, dim, K=40):
    ...
    hm_cen = _nms(hm_cen, kernel = 3) # 下采样的比率3/5
...
def draw_predictions(img, detections, num_classes=3, save_result=False, path=None):
    if save_result:
            ...
            for j in range(num_classes):
                if len(detections[j]) > 0:
                    for det in detections[j]:
                        ...
                        f.write(str(_score))
                        f.write(' ')
                        f.write(str(format(_y/608*50, '.4f')))
                        f.write(' ')
                        f.write(str(format(25-_x/608*50, '.4f')))
                        f.write(' ')
                        f.write(str(format(_w/608*50, '.4f')))
                        f.write(' ')
                        f.write(str(format(_l/608*50, '.4f')))
                        f.write(' ')
                        f.write(str(-_yaw))
                        f.write('\n')
    else:
