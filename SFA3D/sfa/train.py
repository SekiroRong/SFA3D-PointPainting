...
from utils.config import nothing
...
def main():
    ...
    print('configs.distributed',configs.distributed)
...
def main_worker(gpu_idx, configs):
    ...
    if configs.resume_path is not None:
        ...
        model.load_state_dict(torch.load(configs.resume_path, map_location='cuda'))
        if logger is not None:
    ...
    if configs.resume_path is not None:
        ...
        utils_state_dict = torch.load(utils_path, map_location='cuda')
    ...
    for epoch in range(configs.start_epoch, configs.num_epochs + 1):
        ...
        if configs.is_master_node and ((epoch % configs.checkpoint_freq) == 0):
            ...
            # print(utils_state_dict)
            save_checkpoint(configs.checkpoints_dir, configs.saved_fn, model_state_dict, utils_state_dict, epoch, val_loss, lossavg)
