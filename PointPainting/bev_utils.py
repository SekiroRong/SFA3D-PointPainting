...
def pointcloud_to_bev(pointcloud):
    ...
    # shape = (n_points, 1)
    x_bev = np.int_((BEV_HEIGHT)  - pointcloud[:, 2] * descretization_x)
    y_bev = np.int_((BEV_WIDTH/2) - pointcloud[:, 0] * descretization_y)
    z_bev = pointcloud[:, 2] 
...
def clip_pointcloud(pointcloud):
    ...
    mask = np.where((pointcloud[:, 2] >= boundary["minX"]) & (pointcloud[:,2] <= boundary["maxX"]) &
                    (pointcloud[:, 0] >= boundary["minY"]) & (pointcloud[:,0] <= boundary["maxY"])
