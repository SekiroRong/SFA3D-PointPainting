...
from BiSeNetv2.utils.utils import TransformationTrain
...
class KittiSemanticDataset(Dataset):
    def __init__(self, root = 'data/KITTI', split = 'train', mode = 'semantic', transform = None, transform_train=None):
        ...
        if len(images_names) != len(semantic_names):
            print('images semantic Unmatch')
            raise IOError
...
def get_data_set_split(mode, path = r'G:\PP\carla'):
    txt_path = os.path.join(path, 'ImageSets', mode+'.txt')
    dataset = []
...
def create_train_dataloader(root = 'data/KITTI', batch_size = 4):
    ...
    transform_train = TransformationTrain(scales=[1, 1.3], cropsize=[height, width])
    ...
    # train_indices = np.random.choice(indices, train_length, replace=False)
    train_subset = Subset(dataset, train_indices)
    dataloader = DataLoader(train_subset, batch_size, shuffle=True, pin_memory=True)
    return dataloader
...
def create_val_dataloader(root = 'data/KITTI', batch_size = 1):
    ...
    val_length = int(0.8 * len(dataset))
    indices = list(range(0, len(dataset)))
    val_indices = indices[val_length:]
    print('val_length', len(dataset) - val_length)
    # val_indices = np.random.choice(indices, val_length, replace=False)
    val_subset = Subset(dataset, val_indices)
    dataloader = DataLoader(val_subset, batch_size, shuffle=False, pin_memory=True)
    return dataloader
